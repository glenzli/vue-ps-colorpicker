...
export enum ColorMode { RGB = 0, HSV }

export type ColorVariationObject = { mode: ColorMode, c1: number, c2: number, c3: number, alpha: number }
...
export function ColorVariation({ mode = ColorMode.HSV, c1 = 0, c2 = 0, c3 = 0, alpha = 0 } = {}): ColorVariationObject {
  return { mode, c1, c2, c3, alpha }
}
...
export namespace Color$ {
  ...
  const ColorNormalizer = (() => {
    ...
    element.innerHTML = ''
    element.style.cssText = new Array<Array<string>>().concat(['position: absolute', 'pointer-events: none', 'opacity: 0']).join('!important; ')
  ...
  export function HighContrast(color: ColorObject): ColorObject {
    let c = (color.red * 0.299 + color.green * 0.587 + color.blue * 0.114) > 0.5 ? 0 : 1
